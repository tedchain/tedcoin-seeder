...
public:
  ...
  void ParseCommandLine(int argc, char **argv) {
    static const char *help = "Tedcoin-seeder\n"
...
extern "C" void* ThreadDumper(void*) {
  ...
  do {
        ...
        count++;
    {
      ...
      FILE *f = fopen("trkseed.dat.new","w+");
      if (f) {
        {
          CAutoFile cf(f);
          cf << db;
        }
        rename("trkseed.dat.new", "trkseed.dat");
      }
      FILE *d = fopen("trkseed.dump", "w");
...
static const string mainnet_seeds[] = {"node1.tedchain.network", "node2.tedchain.network", "node3.tedchain.network", ""};
...
int main(int argc, char **argv) {
  ...
  FILE *f = fopen("trkseed.dat","r");
  if (f) {
    printf("Loading trkseed.dat...");
    CAutoFile cf(f);
    cf >> db;
    if (opts.fWipeBan)
        db.banned.clear();
